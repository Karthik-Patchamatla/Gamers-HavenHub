...
function GamesTemplate({ platform }) {
    ...
    const fetchGames = async () => {
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/api/games?platform=${encodeURIComponent(platform)}`
        );
        if (!response.ok) {
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`grid bg-[#121212] gap-6 grid-cols-2 md:grid-cols-4 lg:grid-cols-6 ${
      ...
      >
        {games.map((game) => (
          <div
            ...
            className="bg-[#191919] cursor-pointer p-2 rounded-lg"
          >
            <img
            ...
            <span className="font-semibold line-through text-sm text-[#888888] ml-3 my-2">
              {game.oldprice}
            </span>
          </div>
        ))}
